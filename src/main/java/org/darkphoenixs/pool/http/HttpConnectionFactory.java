...
public class HttpConnectionFactory implements ConnectionFactory<HttpURLConnection> {
     ...
     */
    public HttpConnectionFactory(final Proxy proxy, final String address, final String method, final int connectTimeout, final int readTimeout, final Map<String, String> header) {
        this.proxy = proxy;
    ...
    public HttpConnectionFactory(final Properties properties) {
        ...
        for (String headStr : headers.split(",")) header.put(headStr.split(":")[0], headStr.split(":")[1]);
    }
